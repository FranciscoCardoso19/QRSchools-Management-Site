...
try {
    ...
    if (!$id) {
        throw new Exception('ID do equipamentoCategoria não fornecido.');
    }

    $sqlDelete = 'DELETE FROM equipamento_categorias WHERE id = ?';
    if ($stmt = mysqli_prepare($connection, $sqlDelete)) {
        mysqli_stmt_bind_param($stmt, 'i', $id);

        if (mysqli_stmt_execute($stmt)) {
            if (mysqli_stmt_affected_rows($stmt) === 0) {
                throw new Exception('equipamentoCategoria não encontrado ou já removido.');
            }
        } else {
    ...
    echo json_encode([
        'success' => true,
        'message' => 'equipamentoCategoria removido com sucesso.'
    ], JSON_PRETTY_PRINT);
